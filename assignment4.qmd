...
```{sql connection=con}
SELECT count(primary_type), year
FROM crime
...
```{sql connection=con}
SELECT count(arrest), year, district
FROM crime
WHERE year = 2015
OR year = 2016
...
query <- dbSendQuery(con, "SELECT arrest FROM crime WHERE district = 11 AND year = 2016 GROUP BY primary_type ORDER BY count(*) DESC")
...
```{r}
crime %>% 
  select (arrest, district, year, primary_type) %>%
